                         ...
                         const Size &size)
{
  ...
  // system scale factor
  int left = static_cast<int>((screenWidth / 2) - Scale(windowWidth / 2, scale_factor));

  // Calculate window offset from top upper side of the screen depending on current
  // system scale factor
  int top = static_cast<int>((screenHeight / 2) - Scale(windowHeight / 2, scale_factor));

  HWND window = CreateWindow(
      window_class, title.c_str(), WS_OVERLAPPEDWINDOW,
      left, top,
      Scale(windowWidth, scale_factor), Scale(windowHeight, scale_factor),
