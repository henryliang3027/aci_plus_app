...
class Setting18MDUConfigForm extends StatelessWidget {
  const Setting18MDUConfigForm({super.key});
        ...
        return Column(
          ...
          children: [
            ...
            FloatingActionButton(
              ...
              backgroundColor: state.mduConfigs.length < 3
                  ? Theme.of(context).colorScheme.primary.withAlpha(200)
                  : Colors.grey.withAlpha(200),
              onPressed: state.mduConfigs.length < 3
                  ? () async {
                      showAddConfigDialog(groupId: '3').then(
                        (result) async {
...
class _DeviceListView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> buildMDUConfigListView({
      required List<Config> mduConfigs,
    }) {
      String groupId = '3';
      return [
        for (Config mduConfig in mduConfigs) ...[
          configCard(
            config: mduConfig,
            groupId: groupId,
          ),
      ...
      builder: (context, state) {
        return SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...buildMDUConfigListView(mduConfigs: state.mduConfigs),
              const SizedBox(
                height: 20,
              ),
            ],
