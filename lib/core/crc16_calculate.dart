class CRC16 {
  /// Msg:*message to calculate CRC upon
  /// usDatalen: number of bytes in message
  static void calculateCRC16({
    required List<int> command,
    required int usDataLength,
  }) {
    int uchCRCHi = 0xFF; // CRC high byte
    int uchCRCLo = 0xFF; // CRC low byte
    int uIndex;
    int msgIndex = 0;
    int total = usDataLength;

    // pass through message buffer
    while (usDataLength > 0) {
      uIndex = uchCRCHi ^ command[msgIndex]; // calculate the CRC
      msgIndex++;
      uchCRCHi = uchCRCLo ^ _auchCRCHi[uIndex];
      uchCRCLo = _auchCRCLo[uIndex];
      usDataLength--;
    }

    // write back
    command[total] = uchCRCHi;
    command[total + 1] = uchCRCLo;
  }

  static final List<int> _auchCRCHi = [
    
  ];

  //var auchCRCLo[ ]= new Uint8Array(256);
  static final List<int> _auchCRCLo = [
    
  ];
}
