import 'package:aci_plus_app/repositories/amp18_repository.dart';
import 'package:aci_plus_app/repositories/gps_repository.dart';
import 'package:aci_plus_app/setting/bloc/setting18_attribute/setting18_attribute_bloc.dart';
import 'package:aci_plus_app/setting/views/setting18_views/setting18_attribute_view.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

class Setting18AttributePage extends StatelessWidget {
  const Setting18AttributePage({super.key});

  @override
  Widget build(BuildContext context) {
    // return Setting18AttributeView();
    return BlocProvider(
      create: (context) => Setting18AttributeBloc(
        amp18Repository: RepositoryProvider.of<Amp18Repository>(context),
        gpsRepository: RepositoryProvider.of<GPSRepository>(context),
      ),
      child: Setting18AttributeView(),
    );
  }
}
