...
part 'setting18_ccor_node_attribute_event.dart';
part 'setting18_ccor_node_attribute_state.dart';

class Setting18CCorNodeAttributeBloc extends Bloc<
    Setting18CCorNodeAttributeEvent, Setting18CCorNodeAttributeState> {
  Setting18CCorNodeAttributeBloc({
    ...
    required GPSRepository gpsRepository,
        ...
        super(const Setting18CCorNodeAttributeState()) {
    ...
    //     _amp18CCorNodeRepository.forwardCEQStateStream.listen((bool isChanged) {
  ...
  Future<void> _onInitialized(
    Initialized event,
    Emitter<Setting18CCorNodeAttributeState> emit,
  ) async {
    // if (!event.useCache) {
    //   await _amp18CCorNodeRepository.updateSettingCharacteristics();
    // }
  ...
  void _onSettingSubmitted(
    ...
    if (state.coordinates.value != state.initialValues[DataKey.coordinates]) {
      bool resultOfSetCoordinates = await _amp18CCorNodeRepository
          .set1p8GCCorNodeCoordinates(state.coordinates.value);
    ...
    if (firmwareVersion >= 148) {
      ...
      if (userAttributes.isNotEmpty) {
        bool resultOfSetUserAttribute =
            await _amp18CCorNodeRepository.set1p8GCCorNodeUserAttribute(
          technicianID: state.technicianID,
          // inputSignalLevel: state.inputSignalLevel,
          // inputAttenuation: state.inputAttenuation,
          // inputEqualizer: state.inputEqualizer,
